...
public class Solution extends Solver {
  ...
  public String solvePartOne(Stream<String> input) {
    return String.valueOf(
        parseInput(input).filter(p -> pwdIsValidPartOne(p)).count());
  }

  @Override
  public String solvePartTwo(Stream<String> input) {
    return String.valueOf(
        parseInput(input).filter(p -> pwdIsValidPartTwo(p)).count());
  }

  private boolean pwdIsValidPartOne(PasswordEntry p) {
  ...
  private boolean pwdIsValidPartTwo(PasswordEntry p) {
  ...
  private Stream<PasswordEntry> parseInput(Stream<String> input) {
    Stream<PasswordEntry> passwords =
        input.map(l -> {
          String[] line = l.split("[: -]+");
  ...
  final class PasswordEntry {
    ...
    public final int paramOne; /* min count / first character position */
    public final int paramTwo; /* max count / last character position */
